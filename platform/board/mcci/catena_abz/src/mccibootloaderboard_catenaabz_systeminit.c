...
/// the tick count for ABZ platforms
...
/// legacy routine to delay for one ms
static void
delayTick(void)
	{
	McciBootloaderBoard_CatenaAbz_delayMs(1);
	}

void
McciBootloaderBoard_CatenaAbz_handleSysTick(void)
	{
	++s_tickCount;
...
McciBootloaderBoard_CatenaAbz_getMilliseconds(void)
	{
	return s_tickCount;
	}
